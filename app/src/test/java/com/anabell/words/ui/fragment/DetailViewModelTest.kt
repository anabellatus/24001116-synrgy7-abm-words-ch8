...
class DetailViewModelTest {
    ...
    //given
    private val repository = mock<GadgetRepository>()
    private val viewModel = DetailViewModel(
        repository = repository
    )

    private val gadgetObserver = mock<Observer<List<Gadget>>>()

    private val gadgetCaptor = argumentCaptor<List<Gadget>>()

    @Test
    fun retrieveGadgetData() = runTest {
        //given
        val expected = listOf(
            Gadget(
                id = 1,
        ...
        viewModel.gadgets.observeForever(gadgetObserver)

        //when
        whenever(repository.fetchGadgetData()).thenReturn(expected)
        viewModel.retrieveGadgetData()

        //then
        verify(gadgetObserver).onChanged(gadgetCaptor.capture())
        Assert.assertEquals(expected, gadgetCaptor.firstValue)
