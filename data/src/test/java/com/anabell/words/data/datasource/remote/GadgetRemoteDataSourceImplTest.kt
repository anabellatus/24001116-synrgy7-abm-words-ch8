...
class GadgetRemoteDataSourceImplTest {

    //given
    private val gadgetService = mock<GadgetService>()
    private val dataSource = GadgetRemoteDataSourceImpl(
        gadgetService = gadgetService
    )

    @Test
    fun fetchGadgetDataTest() = runTest {
        //given
        val gadgetList = listOf(
            Gadget(
                id = 1,
        ...
        //when
        whenever(gadgetService.getGadgets()).thenReturn(gadgetList)
        val actual = dataSource.fetchGadgetData()

        //then
        Assert.assertEquals(gadgetList, actual)
    }

    @Test
    fun fetchGadgetCategoryDataTest() = runTest {
        //given
        ...
        //when
        whenever(gadgetService.getCategories()).thenReturn(categoryList)
        val actual = dataSource.fetchGadgetCategoryData()

        //then
        Assert.assertEquals(categoryList, actual)
    }
}